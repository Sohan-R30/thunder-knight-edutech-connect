...
const ManageclassNamees = () => {
  const [classNamees, , refetch] = useclassName();
  ...
  const handleDelete = item => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/className/${item._id}`)
          .then(res => {
    ...
    <div>
      <div className="w-full">
        <table className="table">
          ...
          <tbody>
            {
              classNamees.map((item, index) => <tr key={item._id}>
                <th>
                  {index + 1}
                </th>
                <td>
...
export default ManageclassNamees;